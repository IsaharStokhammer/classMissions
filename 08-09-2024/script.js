const data = [
  ...
  let friendsList = data.filter( person => person.friends.find( friend => friend.name === "John Doe") )
  
  // Exercise 2: Use the filter method to get all the people who live in New York
  let liveInNY = data.filter(person => person.address.city === "New York")

  // Exercise 3: Use the filter method to get all the people who are older than 30
  const olderThen30 = data.filter(person => person.age > 30)
  
  // MAP
  
  // Exercise 1: Use the map method to put the names of all the friends of John Doe in a single array (use also the flat method)
  const friendsOfJohn = friendsList.map(person => person.name); 
  console.log(friendsOfJohn.flat())
